...
RENDER_MODE = 'depth'  # 'color', 'depth', 'centers' ('both' although this option only works without additional lines!)

ADDITIONAL_LINES = 'trajectories'  # None, 'trajectories', 'rotations'
...
traj_length = 15 # 15 frames
view_scale = 1.0

def render_to_file(seq, exp, cam_id=9):

    # first define the camera parameters
    ids, extrinsics, intrinsics = get_test_cameras(seq) # Camera parameters for all of the test cameras
    if cam_id is not None and cam_id in ids:
        idx = np.where(ids == cam_id)[0][0]
        w2c = extrinsics[idx]
        k = intrinsics[idx]
    else:
        cam_id = ids[0] # Use the first camera by default if none is specified
        w2c = extrinsics[0]
        k = intrinsics[0]
        # TODO: Enable rendering videos for all cameras in the test set

    if ADDITIONAL_LINES is not None or RENDER_MODE == 'centers':
        o3drender(seq, exp, w2c, k)

    else:
        scene_data, is_fg = load_scene_data(seq, exp)
        num_timesteps = len(scene_data)
        # w2c, k = init_camera(y_angle=0., center_dist=3.8, cam_height=0.8, f_ratio=0.82) # Initialise standard fixed camera if you don't want to use a scene-specific one from the data set itself

        if RENDER_MODE == 'both' or RENDER_MODE == 'color':
            colour_output_file =  f"{pathlib.Path(__file__).parent}/output/{exp}/{seq}/{RENDER_MODE}_render.mp4"
            colour_writer = ffmpeg.write_frames(colour_output_file, size=(w, h), fps=fps, quality=1, codec="png", pix_fmt_out='rgb24')
            colour_writer.send(None)
        if RENDER_MODE == 'both' or RENDER_MODE == 'depth':
            depth_output_file =  f"{pathlib.Path(__file__).parent}/output/{exp}/{seq}/{RENDER_MODE}_render.mp4"
            depth_writer = ffmpeg.write_frames(depth_output_file, size=(w, h), fps=fps, quality=1, codec="png", pix_fmt_in='gray', pix_fmt_out='gray')
            depth_writer.send(None)

        for t in range(num_timesteps):
            im, depth = render(w2c, k, scene_data[t])
            if RENDER_MODE == 'color' or RENDER_MODE == 'both':
                image = (im.cpu().permute(1,2,0).contiguous().numpy() * 255).astype(np.uint8)
                colour_writer.send(image)
            if RENDER_MODE == 'depth' or RENDER_MODE == 'both':
                depth_array = depth.cpu().permute(1,2,0).contiguous().numpy()
                actual_far_range = np.unique(depth_array)[-2]
                scaled_near_range_image = ((depth_array - np.min(depth_array))/(actual_far_range - np.min(depth_array))*245)
                scaled_near_range_image[np.where(scaled_near_range_image > 255)] = 255
                image = scaled_near_range_image.astype(np.uint8).squeeze()
                depth_writer.send(image)

def get_test_cameras(seq):
    md = json.load(open(str(pathlib.Path(__file__).resolve().parent) + f"/data/{seq}/test_meta.json"))  # metadata
    camera_ids = np.array(md['cam_id'][0]) # IDs of the (n) test cameras
...
def o3drender(seq, exp, w2c, k):
    output_file =  f"{pathlib.Path(__file__).parent}/output/{exp}/{seq}/{RENDER_MODE}_{ADDITIONAL_LINES}_render.mp4"
    writer = ffmpeg.write_frames(output_file, size=(w, h), fps=fps, quality=1, codec="png", pix_fmt_out='rgb24')
    writer.send(None)
    ...
    num_timesteps = len(scene_data)
    frames_to_render = range(num_timesteps)
    if ADDITIONAL_LINES == 'trajectories':
        frames_to_render = range(traj_length, num_timesteps)
    ...
    for t in frames_to_render:
        ...
        im_buffer = vis.capture_screen_float_buffer(True)
        image = (np.asarray(im_buffer) * 255).astype(np.uint8)
        writer.send(image)
...
if __name__ == "__main__":
    ...
    for sequence in ["ani_growth", "bending", "branching", "colour", "hole", "rotation", "shedding", "stretching", "translation", "twisting", "uni_growth"]:
        render_to_file(sequence, exp_name)
